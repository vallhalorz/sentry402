...
import type { RiskDossier, Severity } from "@/lib/types";
...
export default function Home() {
  ...
  return (
    <div className="space-y-8">
      <section className="hero-gradient rounded-2xl px-6 py-10 sm:py-14 -mx-6 sm:mx-0">
      ...
      <section className="rounded-xl border border-brand/20 bg-gradient-to-br from-paper-100 to-white p-6 space-y-3 shadow-card">
        ...
        <p className="text-sm text-ink-700 leading-relaxed">
          ...
          integrations call <code className="hash text-xs bg-paper-100 px-1.5 py-0.5 rounded">/api/risk/paid</code> instead. Same response, but x402-gated at $0.05 per
          dossier on Base Sepolia. No API key, no signup, no monthly contract.
        </p>
  ...
  return (
    ...
    >
      <div className="space-y-1.5 flex-1">
        ...
        <p className="hash text-base sm:text-lg break-all">{d.subject.wallet}</p>
          ...
          {[...d.signals]
            ...
            .map((s) => (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {s.fatf_reference && (
                    <RefChip kind="fatf" text={s.fatf_reference} />
                  )}
                  {s.fincen_reference && (
                    <RefChip kind="fincen" text={s.fincen_reference} />
                  )}
                  {s.mica_reference && (
                    <RefChip kind="mica" text={s.mica_reference} />
                  )}
                </div>
  ...
  return (
    <section className="rounded-xl border border-reg-fca/20 bg-reg-fca/5 p-5 text-xs leading-relaxed">
      ...
      <dl className="grid sm:grid-cols-2 gap-x-6 gap-y-1.5 text-ink-700">
        <MetaRow label="Rule pack" value={`${meta.rule_pack_version} · sha256 ${meta.rule_pack_sha256.slice(0, 12)}...`} />
        ...
        <MetaRow label="Generation id" value={meta.generation_id} />
...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline gap-2">
      ...
      <dd className="hash text-ink-700 truncate" title={value}>
        {value}
      </dd>
    </div>
  );
